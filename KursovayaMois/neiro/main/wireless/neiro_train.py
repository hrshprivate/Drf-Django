...
def train_model_by_img(name):
    ...
    if not os.path.exists("dataimageset"):
        print("[ERROR] there is no directory 'dataset'")
        sys.exit()

    known_encodings = []
    images = os.listdir("dataimageset")
    ...
    for(i, image) in enumerate(images):
        ...
        face_img = face_recognition.load_image_file(f"dataimageset/{image}")
        ...
        if len(known_encodings) == 0:
            ...
            for item in range(0, len(known_encodings)):
                ...
                if result[0]:
                    ...
                    print(f"Было обнаружено плохое фото !{image}")
                    break
...
def screen_from_web():
    ...
    # Записываем в файл
    cv2.imwrite('main/wireless/img/cam.jpg', frame)
